<div class="col-lg-12 bg-white">
  <div class="ibox float-e-margins">
    ...
    <h5>Detail Surat Masuk
      <a href="<?=site_url("suratmasukundangan")?>" class="btn btn-danger" style="float:right;margin:-10px 0px"><i class="fa fa-reply"></i> Kembali</a>

    </h5>
    </div>
    <div class="ibox-content">
      <div class="col-lg-6">
        <h4>Detail Undangan Masuk  </h4>
        <?php
          $get=$this->db->query("SELECT * FROM surat_masuk_undangan WHERE id_surat_masuk_undangan='".$id_surat_masuk_undangan."'");
          if($get->num_rows()==0){
      ...
      <div class="col-lg-6">
        ...
        <?php
          $get=$this->db->query("SELECT * FROM disposisi_surat_masuk_undangan a LEFT JOIN bidang b ON a.id_bidang=b.id_bidang WHERE id_surat_masuk_undangan='".$id_surat_masuk_undangan."'");
          if($get->num_rows()>0){
    ...
    <div class="col-lg-12">
      ...
      <form class="validate" enctype="multipart/form-data" method="POST" action="<?php echo site_url('suratmasukundangan/detail/'.$id_surat_masuk_undangan)?>">
        <?php echo $this->session->flashdata('info');?>

        <input type="hidden" value="<?=substr($nomor_agenda,4)?>" name="folder" class="default" />
        <input type="hidden" value="<?=$id_surat_masuk_undangan?>" name="id_from" class="default" />
           <input type="hidden" value="suratmasukundangan" name="jenis_dokumen" class="default" />
    ...
    <div class="col-lg-12">
      ...
      <?php
        $get=$this->db->query("SELECT * FROM dokumen WHERE id_from='$id_surat_masuk_undangan' AND jenis_dokumen='suratmasukundangan'");
        if($get->num_rows()>0){
          foreach ($get->result() as $row) {
            # code...
            ?>
            <div class="col-md-4">
              ...
              <a href="<?=site_url("suratmasukundangan/detail/".$id_surat_masuk_undangan.'?hapus&id='.$row->id_dokumen)?>" onclick="return confirm('Yakin ingin menghapus?')" class="btn btn-danger"><i class="fa fa-edit"></i> Hapus</a>
            </div>
            <?php
          }
        }
